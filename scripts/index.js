...
const deleteTask = (e) => {
  if (!e) e = window.event;//latest event which handle   
  const targetID = e.target.getAttribute("name");
  const type = e.target.tagName;
  const removeTask = state.taskList.filter(({ id }) => id !== targetID);//please add it to the romove task
  state.taskList = removeTask;

  updateLocalStorage();
  if (type === "BUTTON") { //if delete button is pressed then it will return something target the whole element and delete it
    return e.target.parentNode.parentNode.parentNode.parentNode.removeChild(//getting back to more and more steps so that we can delete thw whole task
